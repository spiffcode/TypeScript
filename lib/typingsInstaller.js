...
(function (ts) {
        ...
        function secondaryLookup(dirs) {
            ...
            for (var _i = 0, dirs_1 = dirs; _i < dirs_1.length; _i++) {
                ...
                if (initialLocationForSecondaryLookup !== undefined) {
                    ...
                    var result = loadModuleFromDir(dir, Extensions.DtsOnly, typeReferenceDirectiveName, initialLocationForSecondaryLookup, failedLookupLocations, moduleResolutionState, undefined);
    ...
    function nodeModuleNameResolverWorker(moduleName, containingDirectory, compilerOptions, host, cache, jsOnly) {
        ...
        function tryResolve(extensions) {
            ...
            if (moduleHasNonRelativeName(moduleName)) {
                ...
                var resolved_1 = loadModuleFromDir('node_modules', extensions, moduleName, containingDirectory, failedLookupLocations, state, cache);
    ...
    function loadModuleFromFolder(extensions, moduleName, modulesFolder, modulesFolderExists, failedLookupLocations, state) {
        var candidate = ts.normalizePath(ts.combinePaths(modulesFolder, moduleName));
    ...
    function loadModuleFromDir(dir, extensions, moduleName, directory, failedLookupLocations, state, cache) {
        return loadModuleFromDirWorker(dir, extensions, moduleName, directory, failedLookupLocations, state, false, cache);
    }
    function loadModuleFromDirAtTypes(dir, moduleName, directory, failedLookupLocations, state) {
        return loadModuleFromDirWorker(dir, Extensions.DtsOnly, moduleName, directory, failedLookupLocations, state, true, undefined);
    }
    function loadModuleFromDirWorker(dir, extensions, moduleName, directory, failedLookupLocations, state, typesOnly, cache) {
        ...
        return forEachAncestorDirectory(ts.normalizeSlashes(directory), function (ancestorDirectory) {
            if (ts.getBaseFileName(ancestorDirectory) !== dir) {
    ...
    function loadModuleFromDirOneLevel(dir, extensions, moduleName, directory, failedLookupLocations, state, typesOnly) {
        if (typesOnly === void 0) { typesOnly = false; }
        var modulesFolder = ts.combinePaths(directory, dir);
        var modulesFolderExists = directoryProbablyExists(modulesFolder, state.host);
        if (!modulesFolderExists && state.traceEnabled) {
            trace(state.host, ts.Diagnostics.Directory_0_does_not_exist_skipping_all_lookups_in_it, modulesFolder);
        }
        var packageResult = typesOnly ? undefined : loadModuleFromFolder(extensions, moduleName, modulesFolder, modulesFolderExists, failedLookupLocations, state);
        if (packageResult) {
            return packageResult;
        }
        if (extensions !== Extensions.JavaScript) {
            var nodeModulesAtTypes_1 = ts.combinePaths(modulesFolder, "@types");
            var nodeModulesAtTypesExists = modulesFolderExists;
            if (modulesFolderExists && !directoryProbablyExists(nodeModulesAtTypes_1, state.host)) {
                if (state.traceEnabled) {
                    trace(state.host, ts.Diagnostics.Directory_0_does_not_exist_skipping_all_lookups_in_it, nodeModulesAtTypes_1);
                }
                nodeModulesAtTypesExists = false;
            }
            return loadModuleFromFolder(Extensions.DtsOnly, mangleScopedPackage(moduleName, state), nodeModulesAtTypes_1, nodeModulesAtTypesExists, failedLookupLocations, state);
        }
    }
    ...
    function loadModuleFromGlobalCache(moduleName, projectName, compilerOptions, host, globalCache) {
        ...
        var state = { compilerOptions: compilerOptions, host: host, traceEnabled: traceEnabled };
        var failedLookupLocations = [];
        var resolved = loadModuleFromDirOneLevel('node_modules', Extensions.DtsOnly, moduleName, globalCache, failedLookupLocations, state);
